...
public class ParameterListSyntaxAbstractionTests
{
	[Fact]
	[Trait("Category", "Unit")]
	public void TestAnalyzeNode_Always_ReturnsAnalysisFailure()
	{
		...
		var subject = new ParameterListSyntaxAbstraction(children, null, location);
		var result = subject.AnalyzeNode(state);
		result.IsT1.Should().BeFalse();
		result.T2Value.Location.Should().BeSameAs(location);
		result.T2Value.Reason.Should().Be("Cannot analyze parameter lists");
	}
	...
	public void TestGetExpressionResult_Always_ReturnsAnalysisFailure()
	{
		...
		results.T2Value.Reason.Should().Be("Cannot analyze parameter lists");
	}
}