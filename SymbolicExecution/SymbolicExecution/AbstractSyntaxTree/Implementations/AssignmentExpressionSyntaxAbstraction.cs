...
class AssignmentExpressionSyntaxAbstraction : ExpressionSyntaxAbstraction, IAssignmentExpressionSyntaxAbstraction
{
	...
	public override TaggedUnion<IEnumerable<IAnalysisState>, AnalysisFailure> AnalyzeNode(IAnalysisState previous)
	{
		if (Children.Length != 2)
			return new AnalysisFailure("Assignment expression must have exactly two children", Location.None);

		if (Children[0] is not IIdentifierNameSyntaxAbstraction identifier)
			return new AnalysisFailure("Assignment expression must have an identifier as its first child", Location.None);

		if (identifier.Symbol is not ILocalSymbol localSymbol)
			return new AnalysisFailure("Assignment expression must have a local variable as its first child", Location.None);

		if (Children[1] is not IExpressionSyntaxAbstraction expression)
			return new AnalysisFailure("Assignment expression must have an expression as its second child", Location.None);
	...
	{
		return new AnalysisFailure("Cannot get the result of an assignment expression", Location.None);
	}
}